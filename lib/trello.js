...
export const createTrelloClient = (apiKey, apiToken) => {
  ...
  return {
    ...
    // Trello API は一覧系エンドポイントを1リクエストにつき最大1000件に制限する。
    // ちょうど1000件返った場合は打ち切られている可能性があり、重複判定の母集合が
    // 欠けたまま処理を続けると二重登録を招く。ページング（before/since）は返却順と
    // カーソル基準の対応関係を一次情報で確証できなかったため未実装とし、上限到達時は
    // fetchAllTrelloCards と同じ「母集合が欠けたら全停止」方針で明示的に停止する。
    getCards: async (listId) => {
      const cards = await get(`/lists/${listId}/cards`, { fields: 'name', limit: 1000 });
      if (cards.length >= 1000) {
        throw new Error(
          `リスト ${listId} のカード数がTrello APIの上限（1000件）に達しています。` +
          `これ以上のカードが取得できていない可能性があり、重複判定の精度を保てないため停止しました。` +
          `古いカードをアーカイブ/移動してから再実行してください。`
        );
      }
      return cards;
    },
