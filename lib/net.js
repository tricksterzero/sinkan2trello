...
// サーバー指定を尊重しつつも、異常に長い値（誤設定・過大な指示）でスクリプトが
// 実質無期限に待ち続けないよう、絶対上限でキャップする。
const MAX_RETRY_AFTER_MS = 5 * 60 * 1000;
const parseRetryAfter = (value) => {
  if (!value) return null;
  const sec = Number(value);
  if (Number.isFinite(sec)) return Math.min(Math.max(0, sec * 1000), MAX_RETRY_AFTER_MS);
  const epoch = Date.parse(value);
  return Number.isFinite(epoch) ? Math.min(Math.max(0, epoch - Date.now()), MAX_RETRY_AFTER_MS) : null;
};
...
export const fetchWithRetry = async (
  ...
  // retries<1 だとループが一度も回らず、例外もreturnもせず undefined を返してしまう
  // （「全試行が失敗したら例外を投げる」という契約に反する）。呼び出し側の設定ミスを
  // 早期に気づけるよう、ここで弾く。
  if (!Number.isInteger(retries) || retries < 1) {
    throw new Error(`fetchWithRetry: retries は1以上の整数である必要があります（渡された値: ${retries}）`);
  }
