...
- **多重起動はできません**: 実行中に別プロセスからも起動すると「既に実行中の可能性があります」というエラーになります（同じ本を二重登録しないための安全策）。前回の実行が異常終了して残った場合は20分ほど待つか、プロジェクト直下の `.run.lock` を削除してから再実行してください。
- 対象リストのカード数が Trello API の1リクエストあたりの上限（1000件）に達すると、重複判定の精度を保つため安全のため停止します。古いカードをアーカイブ・移動してから再実行してください。
- 新刊.net側のDESCRIPTION形式が想定と異なるイベント（例: 作者情報の区切りが想定外の件数になっている等）を検知した場合、`!` で始まる警告が表示されます。処理自体は続行されますが、該当の本は作者名などが正しく取得できていない可能性があります。
