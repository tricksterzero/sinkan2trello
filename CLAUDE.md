...
## 残タスク / 検討事項
今後の改善候補（いずれも必須ではなく、着手前にトレードオフを確認すること）。
- **`fetchAllTrelloCards` の全停止**：`Promise.allSettled` で全リストの結果を取り切り、1リストでも失敗があれば**失敗リストIDを名指しして全停止**する（`kind: 'trello-fetch'`、main の catch で専用案内）。「失敗リストのみ警告して残りで続行」案もあるが、重複判定の母集合が欠けると**重複登録が増える**ため、「全停止の方が安全」として据え置き。`allSettled` は全リストのリトライ完遂を待つため、失敗時の停止は最も遅いリストの分だけ遅くなる（診断性を取るための意図的なコスト）。
