...
- **`fetchAllTrelloCards` の全停止**：`Promise.all` で1リストでも取得失敗すると run 全体が止まる。GET リトライ追加で確率は下がったが、「失敗リストのみ警告して残りで続行」する案もある。ただし重複判定の母集合が欠けると**重複登録が増える**ため、現状は「全停止の方が安全」として据え置き。
- **info.js の表示整合**：`index.js`/`setup.js` は `lib/log.js` の進捗・完了ヘルパーで表示を統一済み。`info.js` は未統一なので揃える余地あり。
